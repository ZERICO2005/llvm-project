...
; ruled out.
define double @ret_fpext_negnormal_negsubnormal_mode_dynamic_dynamic(float nofpclass(nan inf zero psub pnorm) %arg0) #0 {
; CHECK-LABEL: define nofpclass(nan inf zero sub pnorm) double @ret_fpext_negnormal_negsubnormal_mode_dynamic_dynamic
; CHECK-SAME: (float nofpclass(nan inf zero psub pnorm) [[ARG0:%.*]]) #[[ATTR1:[0-9]+]] {
; CHECK-NEXT:    [[EXT:%.*]] = fpext float [[ARG0]] to double
; CHECK-NEXT:    ret double [[EXT]]
;
  %ext = fpext float %arg0 to double
...
; TODO: A negative subnormal input may be flushed to +0.0, so +0.0 must not be
; ruled out.
define double @ret_fpext_negnormal_negsubnormal_mode_ftpz_dapz(float nofpclass(nan inf zero psub pnorm) %arg0) #1 {
; CHECK-LABEL: define nofpclass(nan inf zero sub pnorm) double @ret_fpext_negnormal_negsubnormal_mode_ftpz_dapz
